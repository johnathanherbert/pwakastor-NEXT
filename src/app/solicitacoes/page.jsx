...
export default function Home() {
  ...
  const getAtivoStatus = (ativo) => {
    const excipientes = Object.entries(filteredExcipientes).filter(
      ([_, data]) => data.ordens && data.ordens.some((ordem) => ordem.nome === ativo)
    );
  ...
  const isOrdemCompleta = useCallback(
    (ordem) => {
      ...
      return Object.entries(filteredExcipientes).every(([excipient, data]) => {
        ...
        const ordemData = ordens.find((o) => o.id === ordem.id);
  ...
  const isOrdemPesada = (ordem, pesados) => {
    ...
    if (!ordem || !ordem.excipientes || !pesados) return false;
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  <div className="flex items-center gap-2 mt-0.5">
                                    ...
                                    <span className="text-[10px] text-gray-500 dark:text-gray-400">
                                      {ordem.excipientes ? Object.keys(ordem.excipientes).length : 0}{" "}
                                      excipientes
                                    </span>
                                  </div>
                                </div>
